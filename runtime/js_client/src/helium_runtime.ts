...
export interface HeliumResponse<T, E> {
    response?: T;
    error?: E;
}
...
export interface HeliumConfig {
     ...
     */
    get?: (url: string, headers: { [key: string]: string }) => Promise<HeliumResponse<any, any>>;
    /**
     * Function to do the actual request. Defaults to using fetch if not provided
     */
    post?: (url: string, body: any, headers: { [key: string]: string }) => Promise<HeliumResponse<any, any>>;
    /**
...
): Promise<HeliumResponse<any, any>> {
    if (!heliumConfig) {
...
export async function heliumHttpPost(
    ...
    const url = heliumConfig.urlFactory(serviceName, endpointName, namespace);
    const headers = {
        'Content-Type': heliumConfig.encoding === HeliumEncoding.JSON ? 'application/json' : 'application/octet-stream',
    };
    const moreHeaders = heliumConfig.headerFactory ? heliumConfig.headerFactory(url, serviceName, endpointName, namespace) : {};
    if (moreHeaders) {
...
function decodeResponse(
    response: Response,
    successOptions: HeliumMessageFactory<any>,
    errorOptions: HeliumMessageFactory<any>
): Promise<HeliumResponse<any, any>> {
    if (response.headers.get('Content-Type') === 'application/json') {
        ...
        return response.arrayBuffer().then((buffer) => {
            ...
            if (isSuccess) {
                return { response: successOptions?.fromBinary(binaryReader) };
            } else {
                return { error: errorOptions?.fromBinary(binaryReader) } ?? { error: 'Unexpected error' };
            }
        });
    } else {
